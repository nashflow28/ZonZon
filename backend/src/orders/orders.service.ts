...
const CLIENT_OPEN_ORDER_STATUSES: ReadonlySet<OrderStatus> = new Set([
  OrderStatus.PENDING,
  ...ACTIVE_DELIVERY_STATUSES,
]);
const MAX_CLIENT_OPEN_ORDERS = 5;
...
export class OrdersService {
  ...
  async createOrder(clientId: string, dto: CreateOrderDto) {
    ...
    const saved = await this.ordersRepository.manager.transaction(
      async (em) => {
        ...
        const openCount = await em.getRepository(DeliveryOrder).count({
          where: {
            client: { id: clientId },
            status: In([...CLIENT_OPEN_ORDER_STATUSES]),
          },
        });
        if (openCount >= MAX_CLIENT_OPEN_ORDERS) {
          throw new ConflictException(
            `Vous avez déjà ${MAX_CLIENT_OPEN_ORDERS} commandes actives`,
          );
        }
        return em.getRepository(DeliveryOrder).save(order);
      },
    );
  ...
  async computeEta(
    ...
    return {
      ...
      etaMinutes,
      basedOn,
      ...(basedOn === 'driver_position' && position
        ? {
            driverLat: position.lat,
            driverLng: position.lng,
            positionAt: position.updatedAt,
          }
        : {}),
    };
  }
