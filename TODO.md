...
- [x] **Choisir et préparer la nouvelle identité ZonZon** — piste `double-Z mouvement` retenue. Kit de marque V1 produit dans `brand/zonzon-logo-kit/` : masters raster transparents fidèles, variantes horizontale/verticale/symbole/mot-symbole, monochromes, SVG/PDF/EPS plats, TIFF CMJN 300 dpi, formats web, favicon, Android/iOS/PWA, réseaux sociaux, email et documents. Guide PDF, README, manifeste SHA-256 et archive ZIP inclus. Contrôle automatisé : 136 fichiers, 115 images, 0 fichier invalide. *(2026-07-30)*
- [x] **Intégrer le kit — icônes Android, PWA et favicons** *(2026-07-30)* — remplacement mécanique sans retouche. Android : 5 densités de `ic_launcher.png` + **ajout du jeu adaptatif qui manquait** (`ic_launcher_foreground` ×5, `mipmap-anydpi-v26/ic_launcher.xml`, fond `#FFFFFF` / `#0C1A22` en night). PWA : 12 icônes, noms du projet conservés (le manifeste et `web-push.service.ts` les référencent). Favicons admin + PWA. Builds validés : APK release, `ng build` PWA et admin. Vérifié dans l'APK via `aapt2` : icône adaptative active à toutes les densités. **Non déployé.**
- [x] **Kit de marque — versionné et vrai logo intégré** *(2026-07-30)* — `brand/zonzon-logo-kit/` suivi par git (136 fichiers, 20 Mo ; ZIP redondant exclu, `.gitattributes` ajouté pour éviter que git corrompe les PDF/EPS en CRLF). Logo factice remplacé dans l'admin (carré CSS « Z ») et la PWA (nom coloré avec `--zz-go`, une couleur de statut) par le symbole blanc du kit. Jeton `--zz-gold` ajouté. **Admin et PWA déployés.** Bug corrigé au passage : logo étiré sur la PWA (flex column + align-items stretch), détecté en mesurant le DOM en production. Détail : PROGRESS.md session 96.
- [ ] **Kit de marque — reste à arbitrer** — (a) icônes iOS du kit (12 fichiers, renommage nécessaire) : sans objet tant qu'iOS n'est pas distribué ; ~~(b) splash screen Flutter~~ → fait le 2026-07-30 (splash + en-têtes connexion/commerçant/livreur), cf. PROGRESS.md session 98 ; ~~(c) logo horizontal dans les en-têtes~~ → fait le 2026-07-30, cf. PROGRESS.md session 97.
...
- [x] **WhatsApp OTP — opérationnel de bout en bout** *(2026-07-30)* — chantier ouvert en session 88, clos. Entreprise vérifiée, template `zonzon_verification_code` actif, WABA affecté à l'utilisateur système `zonzon-backend`, numéro `+22898596971` enregistré sur la Cloud API, token permanent en place. `POST /auth/forgot-password/request` → `{"sent":true}` et **code reçu sur WhatsApp (confirmé)**. Le « mot de passe oublié » admin est enfin utilisable. 3 erreurs Meta traversées et documentées (vérification d'entreprise, affectation du WABA, `/register`) : voir PROGRESS.md session 93 avant tout futur changement de numéro ou de token.
- [x] **Révoquer et régénérer la clé Firebase Admin** *(sécurité, résolu le 2026-07-30)* — deux clés successives exposées en conversation (lecture du `.env`, puis pièce jointe du JSON). 3ᵉ clé `6e6ecdb3…` injectée via `backend/scripts/set-firebase-credentials.js` (transport par stdin/SSH, jamais affichée), les deux compromises supprimées dans Google Cloud, jeton Google obtenu après révocation. ⚠️ Règle : ne jamais coller ni joindre un secret dans une conversation — voir PROGRESS.md session 93.
- [x] **Créer un SECOND compte ADMIN** *(2026-07-29)* — Kaled YERIMA, `+22890733837`. ⚠️ Créé d'abord en format local (`90828624`), ce qui le rendait introuvable à la connexion et entrait en collision avec le compte CLIENT du même numéro : corrigé, et `scripts/create-admin.js` normalise désormais le numéro et détecte les vrais doublons.
