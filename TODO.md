...
- [~] **WhatsApp OTP — attendre la vérification d'entreprise Meta** *(2026-07-30, ~2 jours ouvrés)* — numéro `+22898596971` enregistré et vérifié sur le WABA de prod `1391869109508703`, moyen de paiement ajouté (n'a PAS suffi), `.env` réparé (`WHATSAPP_ACCESS_TOKEN` vide, `ENABLED=false`). La création du template reste refusée ; vérification d'entreprise soumise, statut « En cours d'examen ». Une fois vérifiée : rejouer le template `zonzon_verification_code` (Authentification / French / « Copier le code »), token permanent (PO), `ENABLED=true`, redémarrage. Détail : PROGRESS.md session 93.
