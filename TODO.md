...
- [ ] **Kit de marque — reste à arbitrer** — (a) versionner ou non `brand/` (20 Mo, dont une archive ZIP de 5 Mo — actuellement non suivi par git) ; (b) **logo dans l'interface admin** : la marque y est aujourd'hui un carré CSS dégradé bleu→émeraude avec la lettre « Z », incompatible avec la charte du kit (bleu nuit → bleu → jaune). Remplacer par le vrai logo est un changement de design, pas une intégration d'actif ; (c) icônes iOS du kit (12 fichiers, renommage nécessaire) — sans objet tant qu'iOS n'est pas distribué ; (d) splash screen Flutter : aucun logo affiché aujourd'hui, ce serait un ajout.
