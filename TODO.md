...
- [~] **WhatsApp OTP — dernière ligne droite : token permanent** *(2026-07-30)* — entreprise **vérifiée** par Meta, template **`zonzon_verification_code` actif** (la vérification était le vrai bloqueur, pas le paiement). Reste, côté PO : générer le token permanent (Utilisateurs système, permissions `whatsapp_business_messaging` + `whatsapp_business_management`), le coller dans `WHATSAPP_ACCESS_TOKEN` du `.env` hôte, passer `WHATSAPP_OTP_ENABLED=true`, `sudo docker compose up -d`, puis tester `POST /auth/forgot-password/request`. Détail : PROGRESS.md session 93.
